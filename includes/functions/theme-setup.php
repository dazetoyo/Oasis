...
function increase_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			...
			'source'    => 'https://www.goodafternoon.ro/plugins/themeforest/advanced-custom-fields-pro.zip',
			'force_activation'   => true,
			'required'  => true,
		),
		...
		array(
			...
			'slug'      => 'js_composer',
			'source'    => 'https://www.goodafternoon.ro/plugins/themeforest/js_composer.zip',
			//'force_activation'   => true,
			'required'  => true,
		),
...
// Apartments sold out to the end of query
function my_pre_get_posts( $query ) {
	...
	if($query->is_main_query()){
		if(
			(isset($query->query_vars['post_type']) && $query->query_vars['post_type'] == 'increase_apartments' )
			||
			is_tax(array('increase_buildings', 'increase_floor','increase_rooms'))
		){
			$query->set('meta_key', 'increase_apartment_sold_out');
